...
def create_graphs(df, timestamps, filtered_states, filtered_accel, filtered_gyro,
                  net_body_accel_estimates, world_accel_estimates, raw_accel, raw_gyro, raw_pressure):
    """
    ...
    def _quat_rotate_vector_batch(qx, qy, qz, qw, vx, vy, vz):
    ...
    with plt.rc_context({
        ...
        # ---------- (2) WORLD ACCEL Z ----------
        # Prefer simulator-provided world linear acceleration (truth), otherwise compute from specific force.
        az_true = None
        if has_true_aw:
            # Direct truth from simulator (vertical world-frame linear acceleration)
            az_true = df['acceleration_true'].to_numpy()
            ax_aw.plot(df['time'], az_true, label='True a_z (world)', color=COLORS['true_aw'], lw=2.0)
        elif has_true_acc and all(c in df.columns for c in ['q_x','q_y','q_z','q_w']):
            # If explicit world acceleration not present, derive:
            # Rotate BODY specific force to WORLD, then add gravity (g_world = [0,0,-9.81]).
            ...
            qz = df['q_z'].to_numpy()
            qw = df['q_w'].to_numpy()
            wx_, wy_, wz_ = _quat_rotate_vector_batch(qx, qy, qz, qw, bx, by, bz)  # f_world = a_world - g_world
            az_true = wz_ - 9.81  # a_world_z = f_world_z + g_world_z; g_world_z = -9.81
            ax_aw.plot(df['time'], az_true, label='True a_z (world)', color=COLORS['true_aw'], lw=2.0)

        # UKF estimate: use the improved world acceleration from UKF state
        az_ukf = world_accel_estimates[:, 2]  # Z component of world acceleration
        az_ukf_s = _safe_smooth(az_ukf, fc=0.6)
        ax_aw.plot(t, az_ukf_s, '--', label='UKF a_z (world)', color=COLORS['ukf_aw'], lw=1.7)
        ...
        # Robust y-limits so truth isn't flattened by spikes
        vals = [az_ukf_s]
        if az_true is not None:
            vals.append(az_true[:len(t)])
        allv = np.concatenate([np.asarray(v) for v in vals])
        lo = np.percentile(allv, 1); hi = np.percentile(allv, 99)
        margin = 0.1 * (hi - lo + 1e-6)
        lo, hi = lo - margin, hi + margin
        # Keep within a sane physical range
        lo = max(lo, -200.0); hi = min(hi, 200.0)
        if hi - lo < 10:
            lo -= 5; hi += 5
        ax_aw.set_ylim(lo, hi)

        if az_true is not None:
            n = min(len(t), len(az_true))
